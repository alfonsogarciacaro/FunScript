...
module internal Literals =
   ...
   [<Literal>]
   let getStringJS = """var byteArray = {0};
   var str = '';
   ...
   [<Literal>]
   let sendRequestJS = """var _method = {0}, 
      _url = {1}, 
      _headerKeys = {2}, 
      _headerValues = {3},
      _body = {4}, 
...
type UTF8Encoding() =
    ...
    static member GetBytes(text : string) : byte[] = failwith "never"

    [<JSEmit(Literals.getStringJS)>]
    static member GetString(bytes : byte[]) : byte[] = failwith "never"
    ...
    member req.AsyncGetResponse() =
        Async.FromContinuations(fun (onSuccess, onError, _) ->
            let onReceived(data : string) =
                let bytes = UTF8Encoding.GetBytes data
                onSuccess(new WebResponse(bytes))
            let onErrorReceived() = onError null
            let body =
                if req.Method = "GET" then null
                else UTF8Encoding.GetString(requestStream.Contents)
            sendRequest(req.Method, url, req.Headers.Keys, req.Headers.Values,
                        (unbox body), onReceived, onErrorReceived)
        )
