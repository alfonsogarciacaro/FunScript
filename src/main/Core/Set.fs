...
type Set<[<EqualityConditionalOn>]'T when 'T : comparison >(comparer:IComparer<'T>, tree: SetTree<'T>) = 
   ...
   new (elements : seq<'T>) = 
      let comparer = Core.GenericComparer<'T>()
   ...
   member s.Map f  : Set<'U> = 
      let comparer = Core.GenericComparer<'U>()
...
module Set =
   ...
   //[<CompiledName("OfList")>]
   let ofList l = Set<_>(List.toSeq l)
