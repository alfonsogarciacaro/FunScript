...
//      | _ -> 
   ...
   let copyToArray s (arr: _[]) i =
      let j = ref i 
      s |> iter (fun x y -> arr.[!j] <- Core.KeyValuePair(x,y); j := !j + 1)
   ...
   let current i =
      if i.started then
            match i.stack with
            | MapOne (k,v) :: _ -> new Core.KeyValuePair<_,_>(k,v)
            | []            -> alreadyFinished()
   ...
   type mkIEnumerator'<'Key,'Value when 'Key: comparison>(s) = 
      ...
      interface IEnumerator<Core.KeyValuePair<'Key,'Value>> with 
...
type Map<[<EqualityConditionalOn>]'Key,[<EqualityConditionalOn;ComparisonConditionalOn>]'Value when 'Key : comparison >
   ...
   new (ie : IEnumerable<_>) =
      let comparer = Core.GenericComparer<'Key>()
...
//            | _ -> false
   ...
   interface IEnumerable<Core.KeyValuePair<'Key, 'Value>> with
   ...
   interface System.IComparable with
      member m.CompareTo(obj: obj) = 
            ...
            Seq.compareWith 
               (fun (kvp1 : Core.KeyValuePair<_,_>) 
                     (kvp2 : Core.KeyValuePair<_,_>)-> 
