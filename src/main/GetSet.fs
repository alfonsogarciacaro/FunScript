   ...
   | Patterns.PropertyGet(target, pi, args) as expr ->
      ...
      if isAutoGenerated pi then
         ...
         | Some target ->
            ...
            |> buildExpr
         | None ->
            if FSharpType.IsModule pi.DeclaringType then                // module Test = let count = 0
               PropertyGet(refType pi.DeclaringType, String pi.Name)
               |> buildExpr
            else
               Expr.Call(pi.GetMethod, args).With(expr.DebugInfo)
               |> (com.CompileExpr >> buildExpr)
   | _ -> None
   ...
   | Patterns.PropertySet(target, pi, args, assignment) as expr ->
      ...
      if isAutoGenerated pi then
         ...
         | Some target ->
            ...
            |> buildExpr   
         | None ->
            if FSharpType.IsModule pi.DeclaringType then                // module Test = let mutable count = 0
               PropertyGet(refType pi.DeclaringType, String pi.Name)
