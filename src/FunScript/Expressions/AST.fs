...
type Range = Microsoft.FSharp.Compiler.Range.range
type FSRef = FSharpMemberOrFunctionOrValue
type FSDeclaration = FSharpImplementationFileDeclaration
type FSValueInfo = FSharpMemberOrFunctionOrValue * FSharpExpr
type FSMemberInfo = FSharpMemberOrFunctionOrValue * FSharpMemberOrFunctionOrValue list list * FSharpExpr
...
//   | Undefined
   ...
   | Lambda of args: FSRef list * body: JSInstruction * isGenerator: bool
...
//              else "ns['" + tname + "']"
      ...
      | Float f     -> removeTrailingZeroes(sprintf "%f" f)
...
type ReturnStrategy =
   | NoReturn
   | Return
   member x.ReturnFrom(dinfo: Range, e: JSExpr) =
      match x with
      | NoReturn -> JSStatement.Do(dinfo, e)
      | Return -> JSStatement.Return(dinfo, e)

type IMappings =
  abstract member Fields: Dictionary<FSharpField, string>
  abstract member Methods: Dictionary<FSRef, string>

type IScopeInfo =
  abstract member ReplaceIfNeeded: FSRef -> FSRef

type ESModuleMember =
  | ESValue of FSRef * JSExpr
  | ESFunction of FSRef * JSStatement
  | ESInitAction of JSStatement
  | ESNestedClass of ESClass
  | ESNestedModule of ESModule

and ESClass =
  { fields: string list
    methods: FSMemberInfo list
    properties: FSMemberInfo list
    events: string list
...
and ESModule =
  { /// Format "folder/subfolder"
    ns: string
    /// If it's a nested module, it will include the route
    name: string
    imports: Dictionary<string, string>
    members: ESModuleMember list }

and ICompiler =
   abstract member ResetImportsCache: unit -> unit
   abstract member ResetMappingsCache: unit -> unit

   abstract member Imports: Dictionary<string, string>
   abstract member Mappings: FSharpEntity -> IMappings

   abstract member CompileCall: IScopeInfo -> FSharpExpr option -> FSRef -> FSharpExpr list -> JSExpr
   ...
   abstract member CompileModule: string -> FSDeclaration list -> ESModule
   abstract member CompileClass: FSharpEntity -> FSDeclaration list -> ESClass
